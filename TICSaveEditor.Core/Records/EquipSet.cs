...
public class EquipSet : INotifyPropertyChanged, IRaisableEntry
{
    ...
    internal EquipSet(UnitSaveData owner, int index)
    {
        _owner = owner;
        Index = index;
    }
    ...
    public string Name
    {
        get => _owner.GetEquipSetName(Index);
        set => _owner.SetEquipSetName(Index, value);
    }

    public byte Job
    {
        get => _owner.GetEquipSetJob(Index);
        set => _owner.SetEquipSetJob(Index, value);
    }

    public bool IsDoubleHand
    {
        get => _owner.GetEquipSetIsDoubleHand(Index);
        set => _owner.SetEquipSetIsDoubleHand(Index, value);
    }
    ...
    /// is NOT empirically verified by SaveDiff (the M5.5 EquipSet fixture changed both regions
    ...
    /// </summary>
    public byte[] RawItemBytes => _owner.GetEquipSetItemBytes(Index);
    ...
    /// </summary>
    public byte[] RawAbilityBytes => _owner.GetEquipSetAbilityBytes(Index);
