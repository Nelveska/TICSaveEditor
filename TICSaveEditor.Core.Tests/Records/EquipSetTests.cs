...
public class EquipSetTests
{
    [Fact]
    public void Index_reflects_position_in_collection()
    {
        var unit = new UnitSaveData(new byte[600]);
        Assert.Equal(0, unit.EquipSets[0].Index);
        Assert.Equal(1, unit.EquipSets[1].Index);
        Assert.Equal(2, unit.EquipSets[2].Index);
    }
    ...
    {
        var unit = new UnitSaveData(new byte[600]);
        unit.EquipSets[1].Name = "Slot1Preset";
        Assert.Equal("Slot1Preset", unit.EquipSets[1].Name);
    }
    ...
    {
        var unit = new UnitSaveData(new byte[600]);
        unit.EquipSets[0].Name = "Foo";

        var output = new byte[600];
        unit.WriteTo(output);

        // EquipSet0 starts at unit-relative offset 0x126; Name occupies bytes [0..65].
    ...
    {
        var unit = new UnitSaveData(new byte[600]);
        unit.EquipSets[0].Name = "Existing";
        unit.EquipSets[0].Name = null!;
        Assert.Equal(string.Empty, unit.EquipSets[0].Name);
    }
    ...
    {
        var unit = new UnitSaveData(new byte[600]);
        unit.EquipSets[2].Job = 0x53;
        Assert.Equal(0x53, unit.EquipSets[2].Job);
    }

    [Fact]
    public void Job_mutation_writes_to_correct_byte_offset()
    {
        // EquipSet1 starts at unit-relative offset 0x126 + 88 = 0x17E.
        // Job lives at section-relative offset 0x56 → unit-relative 0x17E + 0x56 = 0x1D4.
        var unit = new UnitSaveData(new byte[600]);
        unit.EquipSets[1].Job = 0xAB;
    ...
    {
        var unit = new UnitSaveData(new byte[600]);
        Assert.False(unit.EquipSets[0].IsDoubleHand);

        unit.EquipSets[0].IsDoubleHand = true;
        Assert.True(unit.EquipSets[0].IsDoubleHand);

        unit.EquipSets[0].IsDoubleHand = false;
        Assert.False(unit.EquipSets[0].IsDoubleHand);
    }
    ...
    {
        // EquipSet2 starts at 0x126 + 176 = 0x1D6; IsDoubleHand at section-relative 0x57.
        var unit = new UnitSaveData(new byte[600]);
        unit.EquipSets[2].IsDoubleHand = true;
    ...
    public void RawItemBytes_returns_defensive_copy_of_correct_length()
    {
        ...
        var copy = unit.EquipSets[0].RawItemBytes;
        ...
        copy[0] = 0xFF;
        Assert.Equal(0x10, unit.EquipSets[0].RawItemBytes[0]);
    }
    ...
    public void RawAbilityBytes_returns_defensive_copy_at_correct_offset()
    {
        ...
        var copy = unit.EquipSets[0].RawAbilityBytes;
        Assert.Equal(10, copy.Length);
    ...
    public void Setting_Job_fires_PropertyChanged_on_the_entry()
    {
        ...
        Assert.Contains(nameof(EquipSet.Job), fired);
    }
}
